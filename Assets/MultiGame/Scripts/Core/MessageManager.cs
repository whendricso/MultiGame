...
namespace MultiGame {
	...
	public class MessageManager : MonoBehaviour {
		...
		public static void Send(ManagedMessage managedMessage) {
			...
			//at one point, I thought it was a good idea to add a public "Activate" method to make components do their thing. That proved to be less than ideal!
					...
					if (!(_methods.Length < 1)) {
						foreach (MethodInfo info in _methods) {
							...
							if (info.GetParameters().Length < 1) {
								...
								if (info.GetParameters()[0].ParameterType == typeof(int)) {
								...
								if (info.GetParameters()[0].ParameterType == typeof(float)) {
